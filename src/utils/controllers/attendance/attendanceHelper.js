...
exports.calculateSummary = (attendanceData, { scheduleStart, scheduleEnd }) => {
    // Parse attendanceData if it's a string
    const data = typeof attendanceData === 'string' 
        ? JSON.parse(attendanceData) 
        : attendanceData;

    return data.reduce((summary, day) => {
        // Use the correct property names with underscores
        const workHours = parseFloat(day.work_hours) || 0;
        const overtime = parseFloat(day.overtime) || 0;
        const late = parseFloat(day.late) || 0;
        const undertime = parseFloat(day.undertime) || 0;
        const nightDiff = 0;

        // Add to summary
        summary.reg_hrs += workHours;
        summary.total_hours_worked += workHours;
        summary.regular_ot += overtime;
        summary.total_late_time += late;
        summary.total_undertime += undertime;
        summary.total_night_diff += nightDiff;
        summary.worked_days++;

        // Handle rest days and holidays
        if (day.is_rest_day) {
