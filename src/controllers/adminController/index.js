...
exports.createAdminAccount = async (req, res) => {
    ...
    try {
        ...
        // Check if test admin already exists
        const [existing] = await query('SELECT * FROM admin WHERE email = ? OR username = ?', [email, username]);
        
        // if (existing && existing.length > 0) {
        //     await query('DELETE FROM admin WHERE email = ?', [email]);
        //     console.log('Existing test admin removed');
        // }
        ...
        // Get the inserted admin
        const [rows] = await query('SELECT * FROM admin WHERE email = ? OR username = ?', [email, username]);
        const admin = rows[0];
